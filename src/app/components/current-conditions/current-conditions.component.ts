...
export class CurrentConditionsComponent implements OnInit {
	...
	private updateConditionsByZip(
		...
		//* Find the zipcode that is different between the two arrays
		const zipcode = findDifferentElements(
			previousLocations,
			currentLocations
		)[0]; //* Since duplicated zipcodes are avoided, the first position is taken
